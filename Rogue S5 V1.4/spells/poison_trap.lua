...
local menu_elements_pois_trap =
{
    ...
    only_elite_or_boss = checkbox:new(true, get_hash(my_utility.plugin_label .. "poison_trap_only_elite_or_boss_boolean")),
...
local function menu()
    ...
    if menu_elements_pois_trap.tree_tab:push("Poison Trap") then
        ...
        menu_elements_pois_trap.only_elite_or_boss:render("Only Elite or Boss", "")
...
local function is_valid_logics(entity_list, target_selector_data, best_target)
    ...
    if menu_elements_pois_trap.only_elite_or_boss:get() then  
