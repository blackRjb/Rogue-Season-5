...
on_render_menu(function()
    ...
    if menu_elements_bone.andariels_builds:get() then
        spells.barrage.menu()
        spells.dash.menu()
...
on_update(function ()
    ...
    if mode_id == 3 then -- Hybrid
     ...
     local isShadowStepReady = utility.is_spell_ready(355606) -- Assuming this is the Shadow Step spell ID
     local isMeleeReady = utility.is_spell_ready(398258) -- Example: flurry

     if is_elite_or_boss(best_target) and isShadowStepReady and isBarrageReady then
        -- If target is elite/boss/champion and both Shadow Step and Barrage are ready
        if distance_sqr > (7.0 * 7.0) then
            -- Target is far, use Shadow Step to get close
            if cast_spell.target(best_target, 355606, 0.5, false) then
                console.print("Used Shadow Step on elite/boss target")
                cast_end_time = current_time + 0.5
                return
            end
        else
            -- We're already close, use Barrage
            max_range = 7.0 -- Set to melee range for Barrage
        end
     elseif isBarrageReady and distance_sqr > (7.0 * 7.0) then
