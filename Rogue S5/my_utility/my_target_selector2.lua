...
local function get_unit_weight(unit, source)
    local score = 0
    ...
    if source and unit:get_position() then
        local distance = unit:get_position():dist_to(source)
        local distance_factor = 50000 / (1 + distance) 
        score = score + distance_factor
    else
        score = score + 500  -- Vous pouvez ajuster cette valeur selon vos besoins
    end

    return score
end

-- Modification de la fonction get_best_weighted_target pour vérifier la validité de source
local function get_best_weighted_target(entity_list, source)
    local highest_score = -1
    local best_target = nil
    
    if not source then
        -- Si source est nil, on utilise une position par défaut ou on retourne nil
        return nil
    end
    
    for _, unit in ipairs(entity_list) do
        local score = get_unit_weight(unit, source)
