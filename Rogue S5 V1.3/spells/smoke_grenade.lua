...
local menu_elements =
{
    ...
    cast_delay_slider   = slider_float:new(1, 5, 1, get_hash(my_utility.plugin_label .. "smoke_grenade_cast_delay")),
...
local function menu()
    if menu_elements.tree_tab:push("Smoke Grenade") then
        ...
        menu_elements.cast_delay_slider:render("Cast Delay", "Adjust the delay between casts (seconds)",1)


        local dropbox_options = {"No filter", "Elite & Boss Only", "Boss Only"}
        menu_elements.filter_mode:render("Target Filter", dropbox_options, "")
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    local cast_delay = menu_elements.cast_delay_slider:get()
